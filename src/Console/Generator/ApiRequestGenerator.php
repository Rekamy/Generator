            ...
            foreach ($this->db['tables'] as $table) {
                $arrayPaths = [
                    [
                        ...
                        "path" => app_path('Http/Requests/Create' . ucfirst(Str::camel(Str::singular($table->TABLE_NAME)) . 'APIRequest.php'))
                    ],
                    [
                        "view" => view('apitemplate::UpdateApiRequestTemplate')->with('db', (object) $this->db)->with('tablename', $table->TABLE_NAME)->with('context', $this),
                        "path" => app_path('Http/Requests/Update' . ucfirst(Str::camel(Str::singular($table->TABLE_NAME)) . 'APIRequest.php'))
                    ],
                ];
