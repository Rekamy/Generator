...
trait ApiRouteGenerator
{
    public function generateApiRoutes($outputDecorator)
    {
        try {
            ...
            array_push($this->output['rows'], [new TableCell('<info>API Routes</info>')]);
            array_push($this->output['rows'], $separator);

            foreach ($this->db['tables'] as $table) {
                $view = view('apitemplate::CreateAPIRouteTemplate')
